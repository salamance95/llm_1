...
// TODO: 여기에 코드를 쓰세요
let sum15 = 0;
for (const scores of Object.values(scores14)) {
  sum15 += scores;
}
console.log(sum15);
// ───── 문제 16 ───── (개념05 객체 배열)
...
// TODO: 여기에 코드를 쓰세요
let sum18 = 0;
for (const product of cart16) {
  console.log(
    `${product.name}x${product.count}=${product.price * product.count}`,
  );
  sum18 += product.price * product.count;
}
console.log(sum18);
// ───── 문제 19 ───── [도전]
...
// 기대 출력:
// 이서연 91.7
//sum +=average.scores[i]
//sum/average.scores.length
// TODO: 여기에 코드를 쓰세요
let sum = 0;
let Average = 0;
let topAverage = 0;
let topStudent = "";
for (const average of students17) {
  for (let i = 0; i < Object.keys.average; i++) {
    sum += average.scores[i];
  }
